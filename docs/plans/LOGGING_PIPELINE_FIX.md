...
**Fix A — Bind-mount the log directory.** Today logs land in a Docker-managed named volume that only root can read on the Bot-1 host. Switch to a bind-mount at a host path readable/writable by `sgame` without losing the dedicated container UID (`botmanager`, UID 1006 / GID 1007) and without breaking Promtail's tail.
...
| Aspect | Status | Notes |
| --- | --- | --- |
| Bind-mount the log dir | Ready | One-line compose edit + host prep script. No Java change. |
...
1. **Bind-mount path is `/home/sgame/bot-java/logs/` on the host, mapped to `/app/logs` in the bot-manager container and `/logs:ro` in promtail.** Same host path the operator has been (incorrectly) inspecting. The stale `polkitd`-owned files in that directory (May 12-13) must be moved aside as part of the cut-over so the new mount is clean.

2. **Host directory ownership: `1006:1007`.** Numeric matches the container's `botmanager` UID/GID. The `sgame` user must be added to a group with GID 1007 OR the directory must be group-writable AND `sgame` made a member of the host group with GID 1007. Simplest concrete approach: `mkdir -p /home/sgame/bot-java/logs && chown 1006:1007 . && chmod 2775 .` (setgid bit so new files inherit GID 1007), then `usermod -aG 1007 sgame`. The `sgame` user reads/tails as a 1007 group member; the container writes as 1006:1007. **This requires one-time root access** (handled in the host-prep script for Phase 1; the operator coordinates with whoever has root on Bot-1).
...
### Phase 1 — Bind-mount the logs volume (compose + host prep)

**Scope:** Replace the named volume `logs-data` with a bind-mount on the host so `sgame` can `tail`/`ls`/`grep` the live `console.log` without root or `docker exec`.

**Files touched:**
- `/Users/gleb/IdeaProjects/Bot/docker-compose.yml` — change `logs-data:/app/logs` to `./logs:/app/logs`; change `logs-data:/logs:ro` to `./logs:/logs:ro`; remove the top-level `logs-data:` volume declaration.
- `/Users/gleb/IdeaProjects/Bot/deploy.sh` — already does `mkdir -p logs`; add `chown 1006:1007 logs || true` and `chmod 2775 logs || true` lines, guarded by `command -v sudo` since the user may or may not have root. If the chown fails, print a clear warning that an operator with root must run the host-prep step (see below) before the next deploy.
- New file `/Users/gleb/IdeaProjects/Bot/scripts/host-prep-logs.sh` (one-time host-side script):
  - Move `/home/sgame/bot-java/logs/{console.log,console-2026-05-12-10.log}` to `/home/sgame/bot-java/logs.bak.<date>/` (preserve the polkitd-owned stale files for audit, do not delete).
  - `mkdir -p /home/sgame/bot-java/logs`.
  - `chown 1006:1007 /home/sgame/bot-java/logs`.
  - `chmod 2775 /home/sgame/bot-java/logs` (setgid bit so new container-written files inherit GID 1007).
  - `usermod -aG 1007 sgame` (idempotent — re-running is fine).
  - Print final `ls -la` so the operator can confirm.
  - Script must `set -euo pipefail` and refuse to run if not invoked as root.

**Acceptance criteria:**
- After `docker-compose up -d`, the bot-manager container writes to `/app/logs/console.log` and the file appears at `/home/sgame/bot-java/logs/console.log` on the host.
- `ls -la /home/sgame/bot-java/logs/` as `sgame` (no sudo) shows files owned by `1006:1007` with mode `0644`.
- `tail -f /home/sgame/bot-java/logs/console.log` as `sgame` (no sudo) streams the live log.
- Promtail container still tails the file (Promtail container reads via `/logs/console.log`; same in-container path, just backed by the bind-mount now).
...
```bash
# 1. Host-side: confirm bind-mount target exists and is readable by sgame
ssh sgame@Bot-1 'ls -la /home/sgame/bot-java/logs/ && stat -c "%U:%G %a" /home/sgame/bot-java/logs/'
# Expect: directory listing readable, owner shows as 1006:1007 or botmanager:botmanager, mode 2775

# 2. Container-side: bot-manager still sees /app/logs and writes to it
ssh sgame@Bot-1 'docker inspect bot-java-bot-manager-1 --format "{{json .Mounts}}" | python3 -m json.tool | head -20'
# Expect: "Type":"bind" (NOT "volume"), "Source":"/home/sgame/bot-java/logs", "Destination":"/app/logs"

# 3. Live write — tail as sgame with no sudo
ssh sgame@Bot-1 'tail -n 3 /home/sgame/bot-java/logs/console.log'
# Expect: 3 JSON log lines, exit 0

# 4. Promtail still tailing — position keeps pace
ssh sgame@Bot-1 'docker exec bot-java-promtail-1 cat /tmp/positions.yaml'
# Expect: entry for /logs/console.log with byte offset within ~64KB of file size

# 5. Loki ingest healthy
curl -s 'http://Bot-1:3100/loki/api/v1/query?query=count_over_time({job="bot-manager"}[5m])'
# Expect: a positive integer in the "value" field after a bot group has been running for >1 min
```

**Independence:** This phase requires zero Java changes. Once shipped, it remains correct regardless of whether Phases 2–4 ever land.
...
2. As `sgame`, confirm `/home/sgame/bot-java/logs/console.log` is being written:
   ```bash
...
- **Bind-mount and host file ownership.** If the host directory is created by an unprivileged `mkdir` and then the container starts, the container's `botmanager` (1006) will hit `EACCES` on write. The Phase 1 host-prep script MUST run before the first `docker-compose up -d` after the cut-over. The chown in `deploy.sh` is a soft fallback only.
...
The full live verification protocol is documented in **Phase 5** above and is the canonical post-deploy checklist the Releaser must run. Each step has an explicit shell command and expected result. Phases 1, 2, and 3 each carry their own local acceptance commands (also above) that can be run before the Phase 5 live run on Bot-1.

In summary, post-deploy on Bot-1 the Releaser must:

1. `ssh sgame@Bot-1 'ls -la /home/sgame/bot-java/logs/ && tail -n 1 /home/sgame/bot-java/logs/console.log'` — expect listing readable by sgame and one JSON log line. (Validates Fix A.)
2. `ssh sgame@Bot-1 'docker inspect bot-java-bot-manager-1 --format "{{json .Mounts}}"'` — expect `"Type":"bind"` and `"Source":"/home/sgame/bot-java/logs"`.
3. Start a 5-bot group via `POST /api/v1/bot-group/<id>/start`, wait 5 minutes, stop it. Then on Bot-1 compute the ratio `grep -c botGroupId /home/sgame/bot-java/logs/console.log` over `wc -l /home/sgame/bot-java/logs/console.log` — expect ratio ≥ 0.90. (Validates Fix B.)
4. `curl http://Bot-1:3100/loki/api/v1/query?query=count_over_time({job="bot-manager",botGroupId="<id>"}[5m])` should return within 10% of `count_over_time({job="bot-manager"}[5m])` over the same window. (Validates the end-to-end pipeline.)
5. `jq -r 'select(.botGroupId == null) | .thread' /home/sgame/bot-java/logs/console.log | sort | uniq -c | sort -rn | head` — expect residue to be dominated by `multiThreadIoEventLoopGroup-2-N` only (library-internal); any other thread name in the top entries means a callsite was missed in Phase 3.
